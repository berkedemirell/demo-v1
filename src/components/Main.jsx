  ...
  return (
    <div>
      <div className="flex items-center justify-center flex flex-col relative">
        <div
          id="carousel-div"
          className="border border-slate-200 w-deneme xxs:w-deneme-xxs mmd:w-deneme-md xss:w-deneme-xs 3xl:w-deneme-2xl 2xl:w-deneme-xl flex flex-row overflow-hidden mt-6 rounded-md"
        >
          {images.map((i) => {
            return (
              <img
                ...
                src={i.src}
                alt=""
                className="w-car-image xxs:w-car-image-xxs xxs:h-car-image-xxs xss:w-car-image-xs xss:h-car-image-xs 2xl:w-car-image-xl mmd:w-car-image-md mmd:h-car-image-md 2xl:h-car-image-xl 3xl:h-car-image-2xl 3xl:w-car-image-2xl h-car-image"
              />
            );
          })}
        </div>
        <div className="flex flex-row items-center justify-between absolute gap-btn"></div>
          ...
          {images.map((im) => {
            return (
              <button
                ...
                id={im.id - 1}
                key={im.id}
                className={`transition-all duration-500 w-4 h-4 2xl:w-6 2xl:h-6 3xl:w-8 3xl:h-8 border-2 border-main rounded-full ${
                  im.src === activeImg.src ? "bg-main" : ""
                }`}
              ></button>
            );
          })}
